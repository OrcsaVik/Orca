...
public class SpiLoader {
    ...
    public static <T> T getInstance(Class<?> tClass, String key) {
        ...
        if (keyClassMap == null) {
            throw new BizException(ResponseCodeEnum.SPI_NOT_FOUND + tClassName);
        }
        if (!keyClassMap.containsKey(key)) {
            throw new BizException(String.format("SpiLoader 的 %s 不存在 key=%s 的类型", tClassName, key));
        }
        // 获取到要加载的实现类型
